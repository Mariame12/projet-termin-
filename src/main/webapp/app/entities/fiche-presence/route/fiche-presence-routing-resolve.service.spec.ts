jest.mock('@angular/router');

import { TestBed } from '@angular/core/testing';
import { HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRouteSnapshot, Router } from '@angular/router';
import { of } from 'rxjs';

import { IFichePresence, FichePresence } from '../fiche-presence.model';
import { FichePresenceService } from '../service/fiche-presence.service';

import { FichePresenceRoutingResolveService } from './fiche-presence-routing-resolve.service';

describe('Service Tests', () => {
  describe('FichePresence routing resolve service', () => {
    let mockRouter: Router;
    let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
    let routingResolveService: FichePresenceRoutingResolveService;
    let service: FichePresenceService;
    let resultFichePresence: IFichePresence | undefined;

    beforeEach(() => {
      TestBed.configureTestingModule({
        imports: [HttpClientTestingModule],
        providers: [Router, ActivatedRouteSnapshot],
      });
      mockRouter = TestBed.inject(Router);
      mockActivatedRouteSnapshot = TestBed.inject(ActivatedRouteSnapshot);
      routingResolveService = TestBed.inject(FichePresenceRoutingResolveService);
      service = TestBed.inject(FichePresenceService);
      resultFichePresence = undefined;
    });

    describe('resolve', () => {
      it('should return IFichePresence returned by find', () => {
        // GIVEN
        service.find = jest.fn(id => of(new HttpResponse({ body: { id } })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultFichePresence = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultFichePresence).toEqual({ id: 123 });
      });

      it('should return new IFichePresence if id is not provided', () => {
        // GIVEN
        service.find = jest.fn();
        mockActivatedRouteSnapshot.params = {};

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultFichePresence = result;
        });

        // THEN
        expect(service.find).not.toBeCalled();
        expect(resultFichePresence).toEqual(new FichePresence());
      });

      it('should route to 404 page if data not found in server', () => {
        // GIVEN
        jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse({ body: null as unknown as FichePresence })));
        mockActivatedRouteSnapshot.params = { id: 123 };

        // WHEN
        routingResolveService.resolve(mockActivatedRouteSnapshot).subscribe(result => {
          resultFichePresence = result;
        });

        // THEN
        expect(service.find).toBeCalledWith(123);
        expect(resultFichePresence).toEqual(undefined);
        expect(mockRouter.navigate).toHaveBeenCalledWith(['404']);
      });
    });
  });
});
